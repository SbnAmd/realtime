...
void serialize(struct PerformanceEvents *events, char *buffer, int num_events, double* power, unsigned long* energy, float * temperatures) {
    ...
    cJSON_AddItemToObject(perf_counts, "core1", perf1);
    cJSON_AddItemToObject(perf_counts, "core2", perf2);
    cJSON_AddItemToObject(perf_counts, "core3", perf3);
    cJSON_AddItemToObject(perf_counts, "core4", perf4);
